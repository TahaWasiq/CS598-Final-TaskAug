...
class _Operation(nn.Module):
    ...
    #     prob = 0*prob.sum() + prob[torch.arange(batch_size, device=label.device), label.long()]
    #     if self.training:
    #         return RelaxedBernoulli(self.temperature, prob).rsample()
    #     else:
    #         return Bernoulli(prob).sample()

    def get_mask(self, label, batch_size=None):
        prob = torch.sigmoid(self.probability).unsqueeze(0)
        prob = prob.repeat(batch_size, 1)
        # ---> Add the following line to move prob to the right device!
        prob = prob.to(label.device)
        prob = 0*prob.sum() + prob[torch.arange(batch_size, device=label.device), label.long()]
        if self.training:
            return RelaxedBernoulli(self.temperature, prob).rsample()
        else:
...
class RandTemporalWarp(_Operation):
    ...
    def forward(self, input, label):
        mask = self.get_mask(label, input.size(0)).to(input.device)
        BS, C, L = input.shape
        mag = self.magnitude.to(input.device).unsqueeze(0)      # (1, 2)
        mag = mag.repeat(BS, 1)                                 # (BS, 2)
        label = label.to(input.device)                           # (BS,)
        idx = torch.arange(BS, device=label.device)
        mag_rel = mag[idx, label.long()]                         # (BS,)
        mag_rel = mag_rel.view(BS, 1, 1)                         # (BS, 1, 1)
        transformed = self.operation(input, mag_rel, self.warp_obj)
        mask = mask.view(BS, 1, 1)
        retval = (mask * transformed + (1 - mask) * input)
        return retval
